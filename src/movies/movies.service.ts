...
export class MoviesService {
    constructor(
        ...
        @InjectModel(DirectorsMovies) private dirMovieRepository: typeof DirectorsMovies,
        @InjectModel(ActorsMovies) private actorMovieRepository: typeof ActorsMovies,
        @InjectModel(CategoriesMovies) private categMovieRepository: typeof CategoriesMovies,
    ) {}
    ...
    async getAllFilms() {
        const result = await this.moviesRepository.findAll({ include: [
            {
                model: Actors,
                through: {attributes: []}
            },
